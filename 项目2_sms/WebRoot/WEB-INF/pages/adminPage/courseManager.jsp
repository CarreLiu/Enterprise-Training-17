	...
	<script type="text/javascript">
        ...
        //先销毁表格
        	...
        	$('#cusTable').bootstrapTable({
		        ...
		        url:"${pageContext.request.contextPath}/course/findAllCourse.do",//要请求数据的文件路径
        ...
        function actionFormatter(value,row,index){
             let result='';
             if(row.state == 1){
             	result += '<button type="button" class="btn btn-warning btn-xs" onclick="changeState('+row.cid+','+row.state+')">禁用</button>&nbsp;&nbsp;&nbsp;';
             }else{
             	result += '<button type="button" class="btn btn-success btn-xs" onclick="changeState('+row.cid+','+row.state+')">启用</button>&nbsp;&nbsp;&nbsp;';
             }
             result += '<button type="button" class="btn btn-info btn-xs" data-toggle="modal" data-target="#modfiyCourse" onclick="showModalModify('+row.cid+')">修改</button>&nbsp;&nbsp;&nbsp;';
             result += '<button type="button" class="btn btn-danger btn-xs" onclick="showModalDelete('+row.cid+')">删除</button>';
        	 return result;
        }
        ...
        function queryParams(params){
      	...
      	function showModalModify(cid) {
      		$.post('${pageContext.request.contextPath}/course/findById.do', {'cid':cid}, function(course) {
      			$('#primeCname').val(course.cname);
      			$('#cid').val(course.cid);
        ...
        //显示删除确认框
   		...
   		function deleteCourse() {
   			//从隐藏表单域取出cid值
   			let cid = $('#del_id').val();
   			window.location.href = "${pageContext.request.contextPath}/course/deleteCourse.do?cid=" + cid;
   		}
   		...
   		function changeState(cid, state) {
			...
			if(state == '1')
				...
				flag = 1;
			$.post('${pageContext.request.contextPath}/course/modifyCourseState.do', {'cid':cid, 'state':flag}, function() {
		       	...
		       	fields: {
					cname: {
						validators: {
							...
							remote: {
								type: 'post',
		                    	url: '${pageContext.request.contextPath}/course/findByCoursename.do'
							}
						}
					},
					cdesc: {
    ...
    <div class="modal fade" id="modfiyCourse" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      ...
	      <form action="${pageContext.request.contextPath}/course/modifyCourse.do" id="frmModifyCourse" class="form-horizontal" method="post">
	      	  <input type="hidden" id="primeCname">
		      <div class="modal-body">
				<div class="form-group">
			       <label class="col-sm-3 control-label">课程编号：</label>
			       <div class="col-sm-6">
			         <input class="form-control" type="text" value="1" readonly="readonly" name="cid" id="cid">
			       </div>
			    </div>
			     <div class="form-group">
			       <label class="col-sm-3 control-label">课程名称：</label>
			       <div class="col-sm-6">
			         <input class="form-control" type="text" value="java软件开发" name="cname" id="cname">
			       </div>
			    </div>
			     <div class="form-group">
			       <label class="col-sm-3 control-label">课程描述：</label>
			       <div class="col-sm-6">
			         <textarea class="form-control" rows="10" style="resize:none;" name="cdesc" id="cdesc">a、JAVA程序设计
b、SQL SERVER 高级应用
c、jQuery高级编程
d、HTML5与CSS3开发
e、Oracle数据库应用
    ...
    <div class="modal fade" id="addCourse" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      ...
	      <form action="${pageContext.request.contextPath}/course/addCourse.do" id="frmAddCourse" class="form-horizontal" method="post">
		      <div class="modal-body">
			     <div class="form-group">
			       <label class="col-sm-3 control-label">课程名称：</label>
			       <div class="col-sm-6">
			    ...
			    <div class="form-group">
			       <label class="col-sm-3 control-label">课程描述：</label>
			       <div class="col-sm-6">
			         <textarea class="form-control" rows="10" style="resize:none;" name="cdesc"></textarea>
			       </div>
			    </div>
		      </div>
		      <div class="modal-footer">
	...
	<!-- 删除提示模态框start -->
	<div class="modal fade" tabindex="-1" role="dialog" id="deleteCourse">
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
